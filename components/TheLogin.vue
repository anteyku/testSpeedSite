<template>
    <section class="popup popup--login">
        <div class="popup__inner">
            ...
            <form @submit.prevent="onSubmit" class="popup__form popup-form" novalidate>
                <div class="popup-form__wrapper">
                    <p class="contact-error">
                        {{ errors.email }}
                     </p>
                    <input v-bind="email" class="popup-form__input" type="text" placeholder="Email" required>
                    <p class="contact-error">
                        {{ errors.password }}
                     </p>
                    <input v-bind="password" class="popup-form__input" :type="pass" placeholder="Password" required>

                    <div class="seePassword">
                        <Icon @click="seePassword" :class="{see: passActive}" name="material-symbols:disabled-visible" size="30" />
                    </div>
                </div>
                <button class="popup-form__btn popup-form__btn--forgot" type="button">
                ...
                <div class="popup-form__buttons">
                    ...
                    <button class="popup-form__buttons-btn popup-form__buttons-btn--reg btn" type="button">
                        Register
                    </button>
                </div>
            </form>
    ...
    function seePassword(){
        ...
        let onSubmit = handleSubmit( async values =>{
            ...
            snackbar.add({
                    ...
                    await $csrfFetch('/api/loginForm', { method: 'POST', body: values}).then((s)=>{
                            console.log(s);
                    })   
